...
class DisentangledVAE(nn.Module):
    def __init__(self, f_dim=256, z_dim=32, conv_dim=2048, step=256, in_size=64, hidden_dim=512, encode_dims=[48, 32], decode_dims=[64, 48],
        ...
        if self.factorised is True:
            ...
            self.z_lstm = nn.LSTM(self.encode_dims[-1] + self.f_dim, self.hidden_dim, 1, bidirectional=True, batch_first=True)
        ...
        self.decode_lstm = nn.LSTM(self.f_dim + self.z_dim, self.hidden_dim, 1, bidirectional=True, batch_first=True)
    ...
    def decode_trajs(self, zf):
        ...
        lstm_out, _ = self.decode_lstm(zf)
