...
class SVAE_y(nn.Module):
    def __init__(self, f_dim=256, z_dim=32, conv_dim=2048, step=256, in_size=64, hidden_dim=512, encode_dims=[48, 32],
        ...
        self.decode_lstm = nn.LSTM(self.f_dim, self.hidden_dim, 1, bidirectional=True, batch_first=True)
    ...
    def sample_traj(self, batch_size, f=None):
        if f is None:
            f = torch.rand(batch_size, self.f_dim, device=self.device)
            f = f.unsqueeze(1)
        recon_x = self.decode_trajs(f)
        return recon_x
    ...
    def forward(self, x):
        ...
        recon_x = self.decode_trajs(f_expand)
