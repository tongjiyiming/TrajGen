...
class LSTM(nn.Module):
    def __init__(self, f_dim=256, z_dim=32, conv_dim=2048, step=256, in_size=64, hidden_dim=512, encode_dims=[48, 32],
                 decode_dims=[64, 48],
                 traj_len=8, nonlinearity=None, factorised=False, device=torch.device('cpu')):
        ...
        if self.factorised is True:
            ...
            self.z_inter = LinearUnit(self.encode_dims[-1], self.hidden_dim, batchnorm=False)
            self.z_mean = nn.Linear(self.hidden_dim, self.z_dim)
            self.z_logvar = nn.Linear(self.hidden_dim, self.z_dim)
        else:
            self.z_lstm = nn.LSTM(self.encode_dims[-1] + self.f_dim, self.hidden_dim, 1, bidirectional=True,
                                  batch_first=True)
        ...
        self.decode_lstm_cell = nn.LSTMCell(self.encode_dims[-1], self.hidden_dim)
        self.decode_first_x = LinearUnit(self.hidden_dim, self.n_channels)
        # self.decode_lstm = nn.LSTM(self.hidden_dim, self.hidden_dim, 1, bidirectional=False, batch_first=True)
    ...
    def decode_trajs(self, f, h_0, c_0):
        # x = self.deconv_fc(zf)
        ...
        # lstm_out, _ = self.decode_lstm(f, (h_n, c_n))
        # print('lstm_out', lstm_out.shape)
        # xy = self.decode_dense(lstm_out)
        # xy = xy.reshape(-1, self.traj_len, self.n_channels)
        # # print('xy', xy.shape)

        # All states are initially set to 0, especially z_0 = 0
        x_t = self.decode_first_x(f)
        h_t = h_0
        c_t = c_0
        x_out = None

        for _ in range(self.traj_len):
            encoded_x_t = self.encode_dense(x_t)
            h_t, c_t = self.decode_lstm_cell(encoded_x_t, (h_t, c_t))
            x_t = self.decode_dense(h_t)
            if x_out is None:
                # If z_out is none it means z_t is z_1, hence store it in the format [batch_size, 1, z_dim]
                x_out = x_t.unsqueeze(1)
            else:
                # If z_out is not none, z_t is not the initial z and hence append it to the previous z_ts collected in z_out
                x_out = torch.cat((x_out, x_t.unsqueeze(1)), dim=1)
        return x_out
    ...
    def encode_f(self, x):
        lstm_out, (h_n, c_n) = self.f_lstm(x)
        f = lstm_out[:, self.traj_len - 1, 0:self.hidden_dim]
        # mean = self.f_mean(lstm_out)
        # logvar = self.f_logvar(lstm_out)
        mean, logvar = None, None
        return mean, logvar, f, h_n, c_n
    ...
    def sample_traj(self, batch_size, z=None, f_expand=None):
        if z is None:
            f = torch.rand(batch_size, self.f_dim, device=self.device)
            f_expand = f.unsqueeze(1).expand(batch_size, self.traj_len, self.f_dim)
            _, _, z = self.sample_z(batch_size, random_sampling=True)
        zf = torch.cat((z, f_expand), dim=2)
        recon_x = self.decode_trajs(zf)
        return recon_x

    def forward(self, x):
        z_mean, z_logvar, z, z_mean_prior, z_logvar_prior = None, None, None, None, None
        x = self.encode_trajs(x)
        f_mean, f_logvar, f, h_n, c_n = self.encode_f(x)
        # z_mean_prior, z_logvar_prior, _ = self.sample_z(x.size(0), random_sampling=self.training)
        # z_mean, z_logvar, z = self.encode_z(x, f)
        # f_expand = f.unsqueeze(1).expand(-1, self.traj_len, self.f_dim)
        # zf = torch.cat((z, f_expand), dim=2)
        h_n = h_n.squeeze(dim=0)
        c_n = c_n.squeeze(dim=0)
        recon_x = self.decode_trajs(f, h_n, c_n)
